...
function adicionar(){
    if(isNumero(num.value) && !inLista(num.value, valores)){
        ...
        let item = document.createElement('option') // cria um elemento usando o option
        item.text = `Valor ${num.value} adicionado`
        lista.appendChild(item) // adciona o item na lista 
        res.innerHTML = ''

    }else{
        window.alert('valor invalido ou já enconrado na lista')
    }
    num.value = ''
    num.focus() 
}
function finalizar(){
    if (valores.length == 0){
        ...
        res.innerHTML += `<p> A media dos valores inseridos é de ${media}</p>`
